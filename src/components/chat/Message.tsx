...
import { FileText, Download, SmilePlus, Reply, Pencil, Trash2, Copy, Check } from "lucide-react";
import { isImageMime, isAudioMime, formatBytes } from "@/lib/attachments";
...
export function Message({
  ...
  // Sticker-style: a lone emoji (or trio) renders big, no bubble chrome.
  const glyphs = !file && !m.deleted ? Array.from(m.text.trim()) : [];
  const isBigEmoji =
    glyphs.length >= 1 &&
    glyphs.length <= 3 &&
    glyphs.every((g) => /\p{Extended_Pictographic}/u.test(g));
  const rowClass = m.mine ? "flex justify-end" : "flex justify-start";
  const entrance = reduce ? {} : { initial: { opacity: 0, y: 10, scale: 0.97 }, animate: { opacity: 1, y: 0, scale: 1 } };
  if (m.deleted) {
    return (
      <motion.div {...entrance} transition={{ duration: 0.25, ease: "easeOut" }} className={rowClass}>
      ...
      <div className="group relative max-w-[80%]">
        <div
          className={cn(
            ...
            !isBigEmoji &&
              (m.mine
                ? "rounded-tr-md holo text-slate-100 shadow-lg shadow-violet-950/50"
                : "rounded-tl-md peer-glass text-slate-200"),
          )}
        >
          {replySnippet ? (
            <div
              className={cn(
                ...
                m.mine ? "border-cyan-300/50 bg-white/5 text-slate-300" : "border-cyan-300/60 bg-black/30 text-slate-300",
              )}
          ...
          {file && isAudioMime(file.mime) ? (
            <span className="block min-w-52">
              {/* data: URL from allowlisted audio mime, ephemeral relay */}
              <audio controls preload="metadata" src={file.dataUrl} className="h-9 w-full max-w-60" aria-label={`Voice note ${file.name}`} />
              <span className={m.mine ? "mt-0.5 block text-[11px] text-slate-400" : "mt-0.5 block text-[11px] text-slate-400"}>
                {file.name} · {formatBytes(file.size)}
              </span>
            </span>
          ) : null}
          {file && !isImageMime(file.mime) && !isAudioMime(file.mime) ? (
            <a
              href={file.dataUrl}
              download={file.name}
              className={cn(
          ...
          {m.text ? (
            isBigEmoji ? (
              <p className="text-4xl leading-relaxed drop-shadow-lg" aria-label={`Sticker ${m.text}`}>
                {m.text.trim()}
              </p>
            ) : (
              <TranslatableText text={m.text} enabled={!m.mine} />
            )
          ) : null}
          <p className={m.mine ? "mt-0.5 text-[10px] text-slate-400" : "mt-0.5 text-[10px] text-slate-500"}>
