...
import { buzzAlert, buzzMatch, buzzSend } from "@/lib/haptics";
...
/** True when the user is plausibly looking at the conversation right now. */
function isReading(): boolean {
  if (typeof document === "undefined" || typeof window === "undefined") return false;
  return document.visibilityState === "visible" && window.location.pathname === "/chat";
}

/**
 * App-wide realtime state. The provider lives in the root layout, so the
 * socket and the active conversation survive route changes (home, explore,
 * profile…) — leaving /chat no longer hangs up on the stranger.
 * Server stays authoritative: every state change comes from a server frame.
 */
function useMatchmakingState() {
  ...
  // Transmission log for the "stranger left" notice.
  const sessionStartRef = React.useRef(0);
  const msgCountRef = React.useRef(0);
  // Queue insight: gateway-reported line position + transient status notes.
  const [queuePosition, setQueuePosition] = React.useState<number | null>(null);
  const [notice, setNotice] = React.useState<string | null>(null);
  // Unread: messages that landed while you were elsewhere.
  const [unread, setUnread] = React.useState(0);
  // Ref mirror: the session.ended handler must know if we were connected.
  ...
  const ensureConnected = React.useCallback(() => {
    ...
    const offs = [
      client.on("q.searching", (p) => {
        setError(null);
        const pos = (p as { position?: unknown } | undefined)?.position;
        setQueuePosition(typeof pos === "number" && pos > 0 ? pos : null);
      ...
      client.on("session.ended", (p) => {
        ...
        resetTips();
        const wasConnected = stateRef.current.kind === "connected";
        setPeerTyping(false);
        if (reason === "you-left" || reason === "blocked" || reason === "reported") {
          ...
          wasConnected &&
          lastJoin.current &&
          (reason === "peer-next" || reason === "peer-left" || reason === "peer-disconnected")
        ) {
          // Smart queue: the stranger vanished mid-chat — instantly seek the
          // next one with the same prefs (auto = never rematch the same peer).
          const opts = lastJoin.current;
          const secs = Math.max(1, Math.round((Date.now() - sessionStartRef.current) / 1000));
          ...
          setUnread(0);
          lastReadSent.current = null;
          setNotice(`Last transmission: ${n} msg${n === 1 ? "" : "s"} · ${mm}:${ss} — finding someone new…`);
          setState({ kind: "searching" });
          clientRef.current?.send("q.join", { ...opts, auto: true });
        } else {
          setState({ kind: "idle" });
          setError(
            reason === "peer-next"
    ...
    client.connect();
    return offs;
  }, [applyReactions, clearTipTimer, resetTips]);

  // Session-gated lifecycle: connect once signed in, tear down on sign-out
  // (stale identity must never linger). The provider never unmounts, so the
  // socket — and the conversation — survive route changes.
  React.useEffect(() => {
    ...
    const offs = ensureConnected();
    return () => {
      offs?.forEach((off) => off());
    };
  }, [session.status, ensureConnected, resetAll]);

  // Unread clears the moment you're back reading the conversation.
  const clearUnread = React.useCallback(() => setUnread(0), []);
  React.useEffect(() => {
    if (state.kind !== "connected" || !isReading()) return;
    if (unread === 0) return;
    setUnread(0);
  }, [state.kind, messages.length, unread]);

  // Tab-title flash while messages wait elsewhere.
  React.useEffect(() => {
    if (unread === 0 || typeof document === "undefined") return;
    const base = document.title;
    let on = false;
    const t = window.setInterval(() => {
      on = !on;
      document.title = on ? `(${unread}) New message · 0xMingle` : base;
    }, 1200);
    return () => {
  ...
  return { status, state, messages, peerTyping, error, notice, queuePosition, unread, clearUnread, find, stop, next, sendText, sendFile, setTyping, block, report, rtcSend, onRtc, tipIncoming, tipOutgoing, requestTip, respondTip, dismissTips, reactions, toggleReaction, editMessage, deleteMessage, markRead, peerLastReadId, delivered, relayLegacy: relay === "legacy" };
}

export type Matchmaking = ReturnType<typeof useMatchmakingState>;

const MatchmakingContext = React.createContext<Matchmaking | null>(null);

export function MatchmakingProvider({ children }: { children: React.ReactNode }) {
  const value = useMatchmakingState();
...
/** Floating shortcut back to a live conversation (hidden on /chat itself). */
export function ChatReturnPill() {
  const path = usePathname();
  const { state, unread, clearUnread } = useMatchmaking();
  const show = state.kind === "connected" && path !== "/chat";
  return (
    <AnimatePresence>
      {show ? (
        <motion.div
        ...
        >
          <Link
            href="/chat"
            onClick={clearUnread}
            className="relative flex items-center gap-2 rounded-full border border-white/15 bg-[#141524]/95 py-2.5 pl-3 pr-4 text-sm shadow-2xl shadow-violet-950/50 backdrop-blur-md transition-transform hover:scale-105"
            aria-label={`Back to live chat with ${state.kind === "connected" ? state.peer : "stranger"}${unread > 0 ? `, ${unread} unread` : ""}`}
          >
            {unread > 0 ? (
              <span className="absolute -right-1.5 -top-1.5 flex h-5 min-w-5 items-center justify-center rounded-full bg-gradient-to-r from-rose-400 to-orange-300 px-1 text-[10px] font-bold text-black shadow-lg" aria-hidden>
                {unread > 99 ? "99+" : unread}
              </span>
            ) : null}
            ...
            <span className="max-w-[140px] truncate font-medium text-white">
              {state.kind === "connected" ? state.peer : "Stranger"}
            </span>
          </Link>
        </motion.div>
      ) : null}
